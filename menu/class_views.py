...
class SearchListView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):  # get_context_data - когда мы передаем еще дополнительные данные
...
class ProductListView(ListView):
    ...
    paginate_by = 3 # сколько товаров должны быть на одной строке

    def get_queryset(self):
        queryset = super().get_queryset()   # all() ->  Product.objects.all()
...
class ProductDetailView(DetailView):
    model = Product
    # Product.objects.get() -делает такой запрос под копотом
